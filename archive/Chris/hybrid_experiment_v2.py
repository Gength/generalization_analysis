...
import pm4py
import hybrid_algorithm_v2 as algo
...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=3,
        help="Maximum N-gram order for shadow log generation. Backoff from max_n down to 1. Default: 3"
    )
    p.add_argument(
        "--seed",
        type=int,
...
def main():
    ...
    print(f"     N-gram Order (max_n): {args.max_n}")
    ...
    # Check if any result has stratified data (i.e. mutation occurred)
    has_stratified = any(
        r.get('gen_shadow_regular_mean') is not None for r in all_results
    )
    if has_stratified:
        print(f"\n{'─' * 105}")
        ...
        for (miner, w), runs_data in summary.items():
            ...
            if avg_mut_count < 0.5:
                # Too few mutated traces for meaningful stratified analysis
    ...
    with open(out_path, "w") as f:
        json.dump({"version": "2.1", "config": vars(args), "results": all_results}, f, indent=2)
