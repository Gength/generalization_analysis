...
\label{sec:impl:arch}
The metric is implemented in Python on top of PM4Py~\cite{berti2019pm4py}. The code is organized as an \emph{algorithm registry}: each metric version registers itself under a name (\texttt{v1}, \dots, \texttt{v26}), and a benchmark runner loads a version by name and calls a uniform \texttt{evaluate\_miner(log, miner)} entry point. Each version is a \emph{frozen module}. Rather than mutating one file over time, every design stage is preserved as the exact code that produced its benchmark numbers, which is essential for the version study (Sect.~\ref{sec:eval:versions}) and for provenance. The benchmark layer comprises model preparation, per-method bridges (including subprocess bridges to external Java/Docker baselines), the ground-truth scripts, and two runners (\texttt{run\_m1\_family.py} for the metric family, \texttt{version\_comparison.py} for quick multi-seed checks).
...
Adapted and incomplete baselines: M6 uses token-replay fitness in place of the Entropia precision/recall F-measure (Sect.~\ref{sec:impl:baselines}); M3 awaits a per-model stochastic-automaton conversion; M5 has two of $3$--$5$ planned sampling runs; R2 sampled $50$ of $846$ variants on D1.
