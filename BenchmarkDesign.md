...
| — | Sepsis | `data/Sepsis Cases - Event Log_1_all/` | 1,050 | 15,214 | 16 | 846 | 14.5 | 0.2 MB | 0.19 |
...
| — | BPI 2013 Incident | `data/BPI-Challenge_2013/` | 7,554 | 65,533 | 4 | 1,511 | 8.7 | 1.3 MB | 0.80 |
...
**Memory pressure ranking** (estimated, highest to lowest):
1. **BPI 2018** (D4) — 28K variants + 57.4 avg trace length + 2.5M events → worst-case for everything
2. **BPI 2017** (D3) — 15,930 variants + 38.2 avg length → N-gram state explosion for HybridGen, ILP timeout risk
3. **BPI 2019** (D5) — 251K cases → massive PM4Py log objects in RAM
4. **BPI 2013 Incident** (D2) — 7.5K cases, runs fine locally
5. **Sepsis** (D1) — negligible, runs anywhere

---

### Pre-Computed Results Audit

Each external baseline repository was audited for pre-existing benchmark results. **Finding: no pre-computed generalization scores exist for any of our benchmark datasets (D1–D5).** All results must be generated at runtime with full config JSON provenance.

| Method | Local Path | Pre-Computed Results? | Notes |
|--------|-----------|----------------------|-------|
| Entropic Relevance | `src/codebase/jbpt-pm/entropia/` | ❌ None | Guide screenshots show example scores for synthetic logs (`log1.xes`, `model1.pnml`) only. No CSVs. |
| Anti-Alignment | `src/prom_workspace_link/` | ❌ None | No result files found in ProM packages. `AutomataConformance` wraps the call but produces runtime output only. |
| AVATAR | `src/AVATAR/` | ❌ None (for BPI) | Research XLSX files exist but only for synthetic systems (`pa_system_*`, `pb_system_*`), not BPI Challenge logs. |
| Bootstrap Gen | `src/bsgen/` + `src/codebase/jbpt-pm/gen/bootstrap/` | ❌ None | `gen/bootstrap/inputs_r.csv` (42K lines) contains parameter configs referencing Sepsis/Road Traffic/BPI models, but `results.csv` is not pre-computed — it is only generated by running `gen-experiment.jar`. |
| SpeciAL4PM | `src/SpeciAL-core/` | ❌ None | Eval scripts reference BPI 2012, Road Traffic, Sepsis, BPI 2018, BPI 2019 in source code paths, but `out/` and `fig/` directories do not exist — no pre-computed CSVs. |
| Pattern-based Gen | `src/AutomataConformance/` | ❌ None | `PatternGeneralization.java` produces runtime CSV output only. No pre-existing `GeneralizationEvaluationBenchmark.csv` on disk. |
...
**Verdict:** All 8 methods × 7 miners × 5 datasets must be computed from scratch. No cells can be skipped.
